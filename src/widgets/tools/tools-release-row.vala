namespace ProtonPlus.Widgets.Tools {
    public class ReleaseRow : Adw.ActionRow, Utils.ControllerDirectionalFocus {
        ...
        void open_button_clicked () { Utils.System.open_path (job.install_location); }
        public void refresh_usage_pill () { var count = job.tool.group.launcher.get_compatibility_tool_usage_count (job.get_usage_identifier ()); if (count > 0) { usage_pill.set_label (count.to_string ()); usage_pill.set_tooltip_text (ngettext ("Used by %i game", "Used by %i games", count).printf (count)); usage_pill.set_visible (true); } else usage_pill.set_visible (false); }
