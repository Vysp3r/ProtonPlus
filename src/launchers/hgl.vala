namespace ProtonPlus.Launchers {
    public class HeroicGamesLauncher : Models.Launcher {
        public HeroicGamesLauncher (Models.Launcher.InstallationTypes installation_type) {
        ...
        Models.Group[] get_groups () {
            ...
            groups[0] = new Models.Group ("Proton", _("Compatibility tools by Valve for running Windows software on Linux."), "/tools/proton", this);
            groups[0].runners = get_proton_runners (groups[0]);

            groups[1] = new Models.Group ("Wine", _("Compatibility tools for running Windows software on Linux."), "/tools/wine", this);
            groups[1].runners = get_wine_runners (groups[1]);

            groups[2] = new Models.Group ("DXVK", _("Vulkan-based implementation of Direct3D 9, 10 and 11 for Linux/Wine."), "/tools/dxvk", this);
            groups[2].runners = get_dxvk_runners (groups[2]);

            groups[3] = new Models.Group ("VKD3D", _("Variant of Wine's VKD3D which aims to implement the full Direct3D 12 API on top of Vulkan."), "/tools/vkd3d", this);
        ...
        List<Models.Runner> get_proton_runners (Models.Group group) {
            var runners = new List<Models.Runner> ();
        ...
        void install_script (Models.Release release) {
            try {
                string path = "/.config";
        ...
        void uninstall_script (Models.Release release) {
            try {
                string path = "/.config";
                if (release.runner.group.launcher.title.contains ("Flatpak"))path = "/.var/app/com.heroicgameslauncher.hgl/config";
                path = Environment.get_home_dir () + path + "/heroic/store/wine-downloader-info.json";
