namespace AppTests.InstallerTransactionTest {
    ...
    private void test_canceled_download_cleans_workspace () {
        ...
        assert (install (job) == ReturnCode.DOWNLOAD_FAILED); assert (job.canceled); assert (job.state == ProtonPlus.Services.InstallJob.State.NOT_INSTALLED); assert (!FileUtils.test (location, FileTest.EXISTS)); no_entries (cache, ".protonplus-install-"); assert (delete_directory (root));
    }
    private void test_failed_promotion_restores_previous_installation () {
        ...
        no_entries (tools, ".protonplus-previous-"); assert (delete_directory (root));
    }
